      ...
      <div className='w-full max-w-5xl bg-white p-4 rounded'>
        ...
        <form className='my-3 grid gap-3' onSubmit={handleSubmitSubCategory}>
          ...
          <div className='grid gap-1'>
           ...
           <div className='flex flex-wrap gap-2'>
           {
              subCategoryData.category.map((cat)=>{
                return(
                  <p key={cat._id+"selectValue"} className='bg-white shadow-md px-1 m-1 flex items-center gap-2'>
                    {cat.name}
          ...
          <button className={`px-4 py-1 border ${subCategoryData?.name  && subCategoryData?.image && subCategoryData?.category[0]
                            ? "bg-primary-200 hover:bg-primary-100" : "bg-gray-200"} font-semibold`}>Submit
